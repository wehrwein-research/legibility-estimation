...
def get_random_tile(root='/research/wehrwein/borders/bing_maps/', 
                    folder='global-Borders2', no_bing_err=True, 
...
''' ERROR CODES: 
    ...
    Note: If 1 is true, 4-6 will not be returned, yet they are implied 
    Note: If 4 is true, 5-6 will not be returned, yet they are implied
'''
def verify_tile_integrity(img_path, ret_as_list=False, skip_tri_mask=False, 
    ...
    if not img_path.exists():
        if not ret_as_list:
            return 1
        errors.append(1)
        return errors # can't do any more tests without an image
    ...
    if not img_path.with_suffix('.npy').exists():
        if not ret_as_list:
            return 4
        errors.append(4)
        skip_tri_mask = True # cant create tri mask if no .npy file
...
def shuf_file(file, n=None):
    bash = Bash()
    if n is not None:
        files = bash.shuf(f'{file} -n {n}').run().stdout
    else:
        files = bash.shuf(f'{file}').run().stdout
    return files

'''
    given a tile (ex: usa-mex-00000), return the coordinates of the tile 
'''
def get_tile_coords(tile, root):
    import os
    file = f"{root}/{tile[:7]}/{tile}/{tile}.txt"
    coords=None
